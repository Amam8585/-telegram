...
if($authority===''||$pid===''||$amount<=0||$merchant===''){http_response_code(400);exit($TXT['zp_back_bad_request']??'');}
...
$gid=zp_find_plan_by_payer($pid);if(!$gid){http_response_code(404);exit($TXT['zp_back_not_found']??'');}
$st=load_state($gid);if(!$st){http_response_code(404);exit($TXT['zp_back_not_found']??'');}
...
$pmid=(int)($st['pay_msg']['mid']??0);
        if($pmid>0){
        $success_text=$TXT['zp_success_edit_title']??'';
        api('editMessageText',['chat_id'=>$pcid,'message_id'=>$pmid,'text'=>$success_text,'parse_mode'=>'HTML']);
        $btn_text=$TXT['zp_success_button_text']??'';
...
'{card_pan}'=>htmlspecialchars($card?:'-'),
...
$err_prefix=$TXT['zp_verify_error_prefix']??'';
$fallback_fail=$TXT['zp_verify_failed']??'';
$emsg=isset($a['errors']['message'])?($err_prefix.$a['errors']['code'].': '.$a['errors']['message']):$fallback_fail;
echo $emsg;
