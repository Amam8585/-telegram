...
function user_bridge_forward_state($uid,$txt,$lower,&$state,$gid){
    global $TXT;
    $bridge=$state['bridge']??null;
    if(!is_array($bridge)||!($bridge['on']??false))return false;
    $admin_id=(int)($bridge['admin']??0);
    if($admin_id<=0)return false;
    $side=$bridge['side']??'';
    $link_used=is_array($state['link_tokens_used']??null)?$state['link_tokens_used']:['buyer'=>null,'seller'=>null];
    $buyer_ids=array_filter([
        isset($state['buyer_id'])?(string)$state['buyer_id']:null,
        isset($link_used['buyer'])?(string)$link_used['buyer']:null,
    ],function($v){return $v!==null&&$v!=='';});
    $seller_ids=array_filter([
        isset($state['seller_id'])?(string)$state['seller_id']:null,
        isset($link_used['seller'])?(string)$link_used['seller']:null,
    ],function($v){return $v!==null&&$v!=='';});
    if($side==='buyer'&&in_array((string)$uid,$buyer_ids,true)){
        if($lower==='done'){
            $state['bridge']['on']=false;
            if($gid!==0){save_state($gid,$state);}
...
function user_bridge_forward($uid,$txt){
    ...
    foreach($files as $f){
        ...
        $bridge=$state['bridge']??null;
        if(!is_array($bridge)||!($bridge['on']??false))continue;
        $gid=$bridge['gid']??null;
        if(is_string($gid)){$gid=trim($gid);}
        if($gid===null||$gid===''){
            if(preg_match('~/plan_(-?\d+)\.json$~',$f,$mm)){$gid=$mm[1];}else{$gid=0;}
        }
        if(user_bridge_forward_state($uid,$txt,$lower,$state,$gid)){
            return true;
        }
    }
    return false;
...
return;
    ...
    if($ctype==='private'&&$txt){
        ...
        if($ux){
            ...
            $lower_txt=mb_strtolower($txt,'UTF-8');
            if(user_bridge_forward_state($uid,$txt,$lower_txt,$st,$gid)){
                return;
            }
if(($ux['role']??'')==='buyer'){
